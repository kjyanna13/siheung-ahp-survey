...
if st.session_state.page == 1:
    ...
    with st.container(border=True):
        ...
        st.markdown(
         f"""
- 먼저 지역균형발전 핵심과제를 평가할 때 고려해야 할 평가기준의 상대적 중요도({len(CRIT_PAIRS)}문항)를 판단합니다.
- 이후 선택하신 소관 분야의 **전략 및 핵심과제 간 상대적 중요도**를 평가합니다.
- 마지막으로 해당 핵심과제의 **실행 가능성과 파급·연계 효과**를 평가합니다.
- 두 항목의 중요도가 비슷하다고 판단되는 경우에는 '동등'을 선택하시면 됩니다.
- 일관성비율(CR)이 0.10을 넘으면 화면에서 바로 안내하며, 응답 내용을 한 번 더 검토해 주시기 바랍니다.
