...
if st.session_state.page == 1:
    ...
    with st.container(border=True):
        ...
        st.markdown(
            f"""
- **1. 평가기준 중요도:** 평가기준 간 상대적 중요도를 쌍대비교 방식으로 평가합니다. *(총 {len(CRIT_PAIRS)}문항)*
- **2. 전략·핵심과제 중요도:** 선택하신 소관 분야의 전략 및 핵심과제 간 상대적 중요도를 쌍대비교 방식으로 평가합니다.
- **3. 실행가능성·파급효과 평가:** 각 핵심과제의 실행 가능성과 파급·연계 효과를 각각 5점 척도로 평가합니다.
- **쌍대비교 방법:** 제시된 두 항목 중 어느 항목이 더 중요한지 먼저 판단하고, 얼마나 더 중요한지를 선택해 주십시오. 두 항목이 비슷하게 중요하면 '동등(1)'을 선택해 주십시오.
- 일관성비율(CR)이 0.10을 초과하면 화면에서 재검토가 필요한 문항을 안내합니다.
- 전체 응답에는 약 **25~35분**이 소요됩니다.
"""
        )
